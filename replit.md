...
- June 29, 2025: COMPLETED radio station selector final improvements - implemented theme-colored button text and radio icon, light gray dropdown arrow (text-gray-400), dark background (bg-black/80) matching navigation dropdown, custom emoji icons for each station (🎵, 🔥, ⚡, 🤘, 🎸), theme-aware border colors, scrollable dropdown with max-height and custom scrollbar styling, selected station always positioned first in list, perfectly centered volume indicators with proper vertical alignment, dynamic width sizing with 5px padding, automatic closure on scroll events, removed shadow from LIVE IN countdown indicator, cleaned up unused radio player components
