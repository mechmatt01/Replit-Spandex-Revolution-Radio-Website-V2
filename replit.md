...
- June 29, 2025: ENHANCED radio station selector with comprehensive improvements - dark background matching navigation dropdown style, theme-aware border colors instead of prominent white borders, custom station icons (emojis) for each station, centered volume indicator icons vertically within dropdown rows, scrollable dropdown with selected station always positioned first in list, improved visual hierarchy and user experience, theme-colored button text and icon, light gray dropdown arrow, automatic dropdown closure on scroll
