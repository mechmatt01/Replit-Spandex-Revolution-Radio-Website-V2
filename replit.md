...
- July 30, 2025: FIXED LIGHT MODE TEXT VISIBILITY ISSUES - Resolved critical white text on white background visibility problems throughout the site. Fixed Live Statistics and Active Locations sections to display black text in light mode with proper theme-aware styling. Updated navigation brand text "SPANDEX SALVATION" to match hero description text color using colors.textMuted for consistency. Removed all text glow/shadow effects in light mode (textShadow: 'none') while preserving them in dark mode for visual depth. Card backgrounds now properly adapt to themes (white/90 in light mode, zinc-900/50 in dark mode). All text elements now maintain proper contrast ratios for accessibility compliance across all theme modes.
