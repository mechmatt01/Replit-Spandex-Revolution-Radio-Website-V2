...
- July 18, 2025: COMPLETE BUG FIXES FOR RADIO STREAMING WEBSITE - Fixed three critical issues: 1) BORDER COLOR BUG RESOLVED - All ROCK THE AIRWAVES feature boxes and interactive map statistics boxes now properly use dynamic theme colors (colors.primary) for borders, ensuring correct color display across all themes. 2) FULLSCREEN MAP FUNCTIONALITY FIXED - Implemented single map container approach with CSS positioning that transforms between normal and fullscreen modes without duplicating map instances. Map no longer disappears when toggling fullscreen. Removed isFullscreen from useEffect dependencies to prevent map reinitialization. Fullscreen mode properly positions map below navigation bar with smooth transitions. 3) GOOGLE MAPS THEME INTEGRATION CORRECTED - Fixed theme detection to use theme IDs ("classic-metal", "death-metal", etc.) instead of display names ("Classic Metal", "Death Metal"), ensuring proper theme switching between light and dark map styles. All three bugs thoroughly tested and confirmed working.
