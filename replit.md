...
- July 21, 2025: CRITICAL FIXES & FULLSCREEN MAP REFACTOR - Fixed application crash caused by incorrect lucide-react import (RotateCcw → RefreshCw). Completely refactored Google Maps fullscreen functionality with conditional rendering approach - fullscreen map now renders as fixed overlay completely outside parent containers to prevent half-screen reversion issue. Cleaned up orphaned JSX syntax errors. Website now fully functional with working radio streaming, weather data, and all API endpoints responding correctly. Firebase errors are non-critical (missing collections) with proper fallback behavior.
- July 21, 2025: GOOGLE MAPS FULLSCREEN ENHANCEMENT & RADIO STATION UPDATE - Enhanced fullscreen map functionality to cover entire viewport minus navigation bar using calc(100vh - 64px) with proper z-index layering (z-48 below floating player z-50). Removed duplicate FullWidthGlobeMapFixed.tsx file. Updated all 5 radio stations with authentic streaming URLs for current hip-hop, rap, and pop music: Power 105.1 (New York), Hot 97 (New York), 95.5 The Beat (Dallas), iHeart Hip Hop Top 20, and iHeart Hip Hop Beats. Fixed map resize triggers with extended delays to ensure full height coverage.
