...
- June 29, 2025: FINALIZED radio station selector improvements - implemented theme-colored button text and radio icon, light gray dropdown arrow (text-gray-400), dark background (bg-black/80) matching navigation dropdown, custom emoji icons for each station (🎵, 🔥, ⚡, 🤘, 🎸), theme-aware border colors, scrollable dropdown with max-height, selected station always positioned first in list, lighter icon color for current selection (orange-300 to yellow-400 gradient), perfectly centered volume indicators, dynamic width sizing with 5px padding, automatic closure on scroll events
