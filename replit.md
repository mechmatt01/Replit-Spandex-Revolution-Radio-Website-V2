...
- June 29, 2025: FINAL radio station dropdown and LIVE indicator polish - adjusted LIVE IN indicator opacity to 60% for subtle appearance, refined radio selection button border (100% opacity, 2px width, 19px radius), fixed dropdown indicator animation (upward when open, downward when closed), upgraded background blur to backdrop-blur-lg, implemented custom animated SVG speaker icon with staggered sound wave animations, removed unused components for optimized project structure
