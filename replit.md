...
- July 18, 2025: COMPLETE BUG FIXES FOR RADIO STREAMING WEBSITE - Fixed all critical issues: 1) BORDER COLOR BUG RESOLVED - All ROCK THE AIRWAVES feature boxes and interactive map statistics boxes now properly use dynamic theme colors (colors.primary) for borders, ensuring correct color display across all themes. 2) FULLSCREEN MAP FUNCTIONALITY FIXED - Implemented single map container approach with CSS positioning that transforms between normal and fullscreen modes without duplicating map instances. Map properly displays in fullscreen with controls repositioned and header bar with weather display. 3) GOOGLE MAPS THEME INTEGRATION CORRECTED - Fixed theme detection to properly switch between light mode (light map) and all metal themes (dark map). Theme changes now trigger immediate map style updates. 4) LIVE STATISTICS ICON COLORS FIXED - Total listeners and countries icons now properly match theme colors using dynamic filter generation. All bugs thoroughly tested and confirmed working.
