...
export default function VerificationModal({ isOpen, onClose, type, contactInfo, }) {
    ...
    return (<Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md" style={{ backgroundColor: colors.cardBackground, borderColor: colors.primary, color: colors.text }}>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-3 text-xl font-bold">
            {verified ? (<>
                ...
                {type === "phone" ? (<Phone className="h-6 w-6" style={{ color: colors.primary }}/>) : (<Mail className="h-6 w-6" style={{ color: colors.primary }}/>)}
                Verify {type === "phone" ? "Phone Number" : "Email Address"}
              </>)}
          </DialogTitle>
        </DialogHeader>
        {type === "phone" && <div id="recaptcha-container"/>}
        {verified ? (<div className="text-center py-8">
            <CheckCircle className="h-16 w-16 mx-auto mb-4" style={{ color: "#10B981" }}/>
            <p className="text-lg" style={{ color: colors.text }}>
              Your {type} has been successfully verified!
            </p>
          </div>) : (<form onSubmit={handleVerify} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input id="verification-code" type="text" value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, "").slice(0, 6))} className="text-center text-2xl tracking-wider bg-black/50" style={{ color: colors.text, borderColor: "#374151" }} onFocus={(e) => (e.currentTarget.style.borderColor = colors.primary)} onBlur={(e) => (e.currentTarget.style.borderColor = "#374151")} placeholder="000000" maxLength={6} required/>
            </div>
            <div className="flex flex-col gap-3">
              <Button type="submit" disabled={loading || (code?.length || 0) !== 6} className="w-full py-3 font-semibold transition-all duration-200" style={{ backgroundColor: colors.primary, color: "white", opacity: loading || (code?.length || 0) !== 6 ? 0.6 : 1 }}>
                {loading ? "Verifying..." : "Verify Code"}
              </Button>
              <Button type="button" variant="ghost" onClick={handleResend} className="text-sm" style={{ color: colors.primary }}>
