...
export default function Schedule() {
    ...
    return (<section id="schedule" className="py-20 transition-colors duration-300" style={{ backgroundColor: colors.background }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {/* Weekly Schedule */}
          <div>
            <h3 className="font-black text-xl mb-6 text-center" style={{ color: "var(--color-primary)" }}>
              This Week's Lineup
            </h3>
            <div className="space-y-4">
              {isLoadingShows ? (
        // Show skeleton loading cards
        Array.from({ length: 6 }).map((_, index) => (<Card key={index} className="p-6" style={{ backgroundColor: colors.card }}>
                  ...
                  </Card>))) : ((filteredWeeklyShows || []).map((show) => (<div key={show.id} className="relative">
                  {/* Glow Effect */}
                  <div className="absolute inset-2 rounded-2xl overflow-hidden">
                    <div className="absolute inset-0 blur-xl opacity-0 transition-opacity duration-300" style={{
                background: `linear-gradient(135deg, ${colors.primary}, ${colors.secondary || colors.primary})`,
                opacity: hoveredShow === `weekly-${show.id}` ? 0.3 : 0,
            }}/>
                  </div>
                  
                  <Card className="relative group cursor-pointer transition-all duration-300 border-2 p-4 enhanced-glow show-container" style={{
                backgroundColor: colors.background,
                borderColor: colors.border,
                boxShadow: `${colors.border}20 0px 8px 32px`,
                height: "160px",
                transform: hoveredShow === `weekly-${show.id}` ? 'scale(1.05)' : 'scale(1)',
            }} onClick={() => setSelectedWeeklyShow(show)} onMouseEnter={() => setHoveredShow(`weekly-${show.id}`)} onMouseLeave={() => setHoveredShow(null)}>
                ...
                flexDirection: "column",
                    ...
                    <div className="flex items-center justify-center space-x-2" style={{ marginTop: "auto", paddingTop: "8px" }}>
          ...
          {(pastShows?.length || 0) > 0 && (<div>
            ...
            Array.from({ length: 3 }).map((_, index) => (<Card key={index} className="p-6" style={{ backgroundColor: colors.card }}>
                      <div className="flex items-center justify-between mb-4">
                    ...
                    </Card>))) : ((pastShows || []).slice(0, 3).map((show) => (<div key={show.id} className="relative">
                    {/* Glow Effect */}
                    <div className="absolute inset-2 rounded-2xl overflow-hidden">
                      <div className="absolute inset-0 blur-xl opacity-0 transition-opacity duration-300" style={{
                ...
                }} onClick={() => setSelectedPastShow(show)} onMouseEnter={() => setHoveredShow(`past-${show.id}`)} onMouseLeave={() => setHoveredShow(null)}>
                    <CardContent className="p-0" style={{
                    height: "100%",
                    display: "flex",
                    flexDirection: "row",
                    ...
                    paddingLeft: "16px"
                        ...
                        <Button className="rounded-full transition-all duration-300 hover:scale-110" style={{
        ...
        <div className="mt-12 flex justify-center w-full">
          <Button className="px-6 py-3 rounded-full font-bold transition-all duration-300 border-0" style={{
            backgroundColor: colors.primary,
      ...
      {selectedWeeklyShow && (<div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4" onClick={() => setSelectedWeeklyShow(null)} style={{
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                minHeight: '100vh',
          ...
          <div className="bg-background/95 backdrop-blur-sm border-2 rounded-xl p-8 max-w-md w-full relative show-popup-content" style={{
            ...
            {/* Close Button */}
            <button onClick={() => setSelectedWeeklyShow(null)} className="absolute top-4 right-4 p-2 rounded-full transition-all duration-200 hover:scale-110 flex items-center justify-center" style={{
            ...
            }} title="Close show details" aria-label="Close show details">
              <X className="h-5 w-5"/>
            </button>
            ...
            <div className="text-center space-y-4">
              {/* Title */}
              <h2 className="font-black text-2xl" style={{ color: colors.text }}>
      ...
      {selectedPastShow && (<div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4" onClick={() => setSelectedPastShow(null)} style={{
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                minHeight: '100vh',
          ...
          <div className="bg-background/95 backdrop-blur-sm border-2 rounded-xl p-8 max-w-md w-full relative show-popup-content" style={{
            ...
            {/* Close Button */}
            <button onClick={() => setSelectedPastShow(null)} className="absolute top-4 right-4 p-2 rounded-full transition-all duration-200 hover:scale-110 flex items-center justify-center" style={{
                color: colors.primary,
                backgroundColor: `${colors.primary}20`,
                width: "40px",
                height: "40px",
