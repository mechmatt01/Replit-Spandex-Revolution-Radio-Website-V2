...
export default function Schedule() {
    ...
    const { data: weeklyShows = [], isLoading: isLoadingShows } = useQuery({
        queryKey: ["/api/schedules"],
    });
    const { data: pastShows = [], isLoading: isLoadingPast } = useQuery({
    ...
    return (<section id="schedule" className="py-20 transition-colors duration-300" style={{ backgroundColor: colors.background }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        Array.from({ length: 6 }).map((_, index) => (<Card key={index} className="p-6" style={{ backgroundColor: colors.card }}>
                  ...
                  </Card>))) : (filteredWeeklyShows.map((show) => (<Card key={show.id} className="group cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-2xl border-2 p-4" style={{
          ...
          {pastShows.length > 0 && (<div>
            ...
            Array.from({ length: 3 }).map((_, index) => (<Card key={index} className="p-6" style={{ backgroundColor: colors.card }}>
                    ...
                    </Card>))) : (pastShows.slice(0, 3).map((show) => (<Card key={show.id} className="group cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-2xl border-2 p-6" style={{
                    backgroundColor: colors.background,
                    borderColor: `${colors.primary}80`,
                    boxShadow: `0 8px 32px ${colors.primary}20`,
                    ...
                    gap: "12px",
                      ...
                      {/* Description */}
                      <p className="text-sm font-semibold text-center text-gray-400" style={{
                    minHeight: "40px",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center"
                      ...
                      <Button className="mx-auto rounded-full transition-all duration-300 hover:scale-110" style={{
                    ...
                    handlePastShowSelect(show);
                      ...
                      {/* Date with Duration */}
                      <div className="flex items-center justify-center space-x-2" style={{ marginTop: "14px" }}>
                        <Calendar className="text-gray-500 h-3 w-3"/>
                        <span className="text-gray-500 text-xs font-bold">
        ...
        <div className="mt-12 flex justify-center w-full">
          <Button variant="outline" className="border-2 px-6 py-3 rounded-full font-bold transition-all duration-200 hover:scale-105" style={{
      ...
      {selectedWeeklyShow && (<div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50" onClick={() => setSelectedWeeklyShow(null)}>
          <div className="bg-background/95 backdrop-blur-sm border-2 rounded-xl p-8 max-w-md w-full mx-4 relative" style={{
                borderColor: colors.primary,
                backgroundColor: colors.background,
