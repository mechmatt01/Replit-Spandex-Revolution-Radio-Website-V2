...
export default function Subscription() {
    ...
    return (<section id="subscribe" className="py-20 bg-dark-surface">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="md:hidden flex flex-col gap-8 max-w-sm mx-auto">
          {subscriptionTiers.map((tier, index) => (<div key={`mobile-${tier.name}`}>
              <h3 className={`font-black text-white mb-4 text-center ${tier.color === "metal-gold"
                ? "text-metal-gold"
                : tier.color === "metal-red"
                    ? "text-metal-red"
                    : "text-white"}`} style={{ fontSize: "1.25rem" }}>
                {tier.name}
              </h3>

              <div className="rounded-lg flex flex-col transition-all duration-300 relative" style={{
                minHeight: "540px",
                ...
                {tier.popular && (<div className="absolute left-1/2 transform -translate-x-1/2 px-4 py-1 rounded-full text-xs font-bold" style={{
                ...
                <div className="p-8 flex flex-col h-full justify-between">
                  <div className="text-center mb-6">
                    <div className={`text-3xl font-bold mb-1 ${tier.color === "metal-gold"
                ? "text-metal-gold"
                : tier.color === "metal-red"
                    ? "text-metal-red"
                    : "text-metal-orange"}`}>
                  ...
                  <ul className={`space-y-3 ${tier.popular ? "mb-6" : "mb-8"} flex-grow`}>
                    {tier.features.map((feature, featureIndex) => (<li key={featureIndex} className="flex items-center justify-center text-gray-300">
                        <Check className="w-5 h-5 text-metal-orange mr-3 flex-shrink-0"/>
                        <span className="text-sm text-center">{feature}</span>
                      </li>))}
                  </ul>

                  <Button onClick={() => handleSubscribe(tier.name)} className="w-full py-3 text-lg font-semibold" style={{
        ...
        <div className="hidden md:block relative mx-auto" style={{
          ...
          {subscriptionTiers.map((tier, index) => (<div key={`desktop-${tier.name}`} className="absolute transition-all duration-300" style={{
                width: "320px",
                left: index === 0
              ...
              <div className="rounded-lg flex flex-col transition-all duration-300 relative overflow-hidden" style={{
                ...
                {tier.popular && (<>
                    <div className="absolute left-1/2 transform -translate-x-1/2 px-4 py-1 rounded-full text-xs font-bold" style={{
                    top: "-12px", // Centered over top border
                    background: "linear-gradient(135deg, #ff6b35, #f7931e)",
                    color: "black",
                    whiteSpace: "nowrap",
                ...
                : tier.color === "metal-red"
                  ...
                  <ul className={`space-y-3 ${tier.popular ? "mb-6" : "mb-8"} flex-grow`}>
                    {tier.features.map((feature, featureIndex) => (<li key={featureIndex} className="flex items-start text-gray-300">
        ...
        <DialogContent className="bg-dark-surface border-dark-border">
          ...
          <form onSubmit={handleConfirmSubscription} className="space-y-4">
            <div>
              ...
              <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email" className="bg-dark-bg border-dark-border text-white" required/>
            </div>
