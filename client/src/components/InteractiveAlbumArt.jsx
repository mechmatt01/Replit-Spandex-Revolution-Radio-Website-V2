...
export default function InteractiveAlbumArt({ artwork, title, artist, isPlaying = false, size = "md", className = "", }) {
    ...
    return (<div className={`relative ${sizeClasses[size]} rounded-xl overflow-hidden shadow-lg cursor-pointer transition-all duration-300 ${className}`} onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} style={{
      ...
      <div className="absolute inset-0 flex items-center justify-center transition-all duration-500" style={{
        ...
        <div className="transition-transform duration-300" style={{ transform: isHovered ? "rotate(5deg)" : "rotate(0deg)" }}>
          {artwork === "advertisement" ||
            ...
            title.toLowerCase().includes("commercial break") ? (<div className={`text-white font-black bg-red-600 rounded m-1 flex items-center justify-center ${size === 'sm'
                ? 'text-xs px-1 py-0.5 w-6 h-4' // 1.5x smaller for floating player
                : 'text-2xl px-4 py-2 w-16 h-12' // 3x larger for main player
            }`}>
              AD
            </div>) : (<ThemedMusicLogo size={logoSizes[size]}/>)}
        </div>
      </div>

      {/* Album Artwork with Verification */}
      {artwork && artwork.trim() && artwork !== "advertisement" && (<div className="absolute inset-0 transition-all duration-500" style={{
                opacity: imageLoaded ? 1 : 0,
                transform: isHovered ? "scale(1.05)" : "scale(1)",
            }}>
          <img src={artwork} alt={`${title} by ${artist}`} className="w-full h-full object-cover" onLoad={handleImageLoad} onError={handleImageError} referrerPolicy="no-referrer" crossOrigin="anonymous"/>
          ...
          {/* Reflection effect */}
          {isHovered && (<div className="absolute inset-0 transition-opacity duration-300" style={{
      ...
      <div className="absolute inset-0 rounded-xl transition-all duration-300 pointer-events-none" style={{
            border: isHovered
      ...
      {artwork && !imageLoaded && (<div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent animate-pulse"/>)}
    </div>);
}
