...
export default function AvatarSelector({ isOpen, onClose, currentAvatar, onAvatarUpdate, }) {
    ...
    return (<Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-2xl max-h-[80vh] overflow-y-auto" style={{
          ...
          <TabsContent value="avatars">
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-5 gap-4">
                {FREE_AVATAR_OPTIONS.map((avatar) => (<div key={avatar.id} className={`relative cursor-pointer rounded-lg p-2 transition-all duration-200 hover:scale-105`} style={{}} onClick={() => setSelectedAvatar(avatar.url)}>
                    <div className="aspect-square rounded-lg overflow-hidden bg-gray-800">
                      <img src={avatar.url} alt={avatar.name} className="w-full h-full object-cover" onError={(e) => {
                ...
                target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(avatar.name)}&background=f97316&color=fff&size=128`;
                    ...
                    {selectedAvatar === avatar.url && (<div className="absolute -top-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center" style={{ backgroundColor: colors.primary }}>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-5 gap-4">
                {PREMIUM_AVATAR_OPTIONS.map((avatar) => (<div key={avatar.id} className={`relative cursor-pointer rounded-lg border-2 p-2 transition-all duration-200 hover:scale-105 ${selectedAvatar === avatar.url
                ? "border-primary"
                : "border-yellow-600"}`} style={{
                ...
                background: "linear-gradient(135deg, rgba(255, 215, 0, 0.1), rgba(255, 165, 0, 0.05))",
                    ...
                    <div className={`aspect-square rounded-lg ${avatar.jumpingElements ? 'premium-avatar-container' : 'overflow-hidden'} bg-gradient-to-br from-gray-800 to-gray-900 relative`}>
                      <img src={avatar.url} alt={avatar.name} className="w-full h-full object-cover rounded-lg" style={{
          ...
          <TabsContent value="upload">
            <div className="space-y-6">
              <div className="text-center">
                <div className="mx-auto w-32 h-32 rounded-full flex items-center justify-center cursor-pointer transition-colors" onClick={() => fileInputRef.current?.click()} style={{}}>
                  {selectedAvatar && selectedAvatar.startsWith("data:") ? (<img src={selectedAvatar} alt="Uploaded avatar" className="w-full h-full rounded-full object-cover"/>) : selectedAvatar &&
            !selectedAvatar.startsWith("/avatars/") ? (<img src={selectedAvatar} alt="Current avatar" className="w-full h-full rounded-full object-cover"/>) : (<div className="text-center">
                ...
                <Button type="button" variant="outline" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="mt-4" style={{ borderColor: colors.primary, color: colors.primary }}>
        ...
        <div className="flex justify-between pt-4 border-t" style={{ borderColor: "#374151" }}>
          <Button type="button" variant="ghost" onClick={onClose} style={{ color: colors.text }}>
            Cancel
          </Button>

          <Button type="button" onClick={handleSave} disabled={!selectedAvatar || saving} className="px-6" style={{
