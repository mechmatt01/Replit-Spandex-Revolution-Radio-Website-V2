...
const FullWidthGlobeMapFixed = () => {
    ...
    const fetchActiveListeners = async () => {
        try {
            ...
            if (response.ok) {
                ...
                setActiveListeners(data.listeners.filter((listener) => listener.IsActiveListening && listener.Location));
            }
        }
        catch (error) {
    ...
    const createListenerMarker = (listener, mapInstance) => {
        const marker = new google.maps.Marker({
    ...
    return (<>
      ...
      {isFullscreen && (<div className="fixed bg-black/60 backdrop-blur-sm z-[9998] transition-all duration-500 ease-in-out" style={{
      ...
      <section className={`${isDarkMode ? "bg-black" : "bg-white"} py-20`}>
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className={`text-center mb-16 transition-all duration-500 ease-in-out ${isFullscreen ? 'opacity-0 invisible' : 'opacity-100 visible'}`}>
            ...
            <div className="mb-6 relative max-w-lg mx-auto">
              ...
              <div className="relative rounded-2xl backdrop-blur-md border border-primary/10 shadow-2xl overflow-hidden transition-all duration-500 ease-in-out" style={{
            ...
            boxShadow: `0 8px 32px ${adaptiveTheme.accentColor}20, 0 0 0 1px ${adaptiveTheme.accentColor}10`
                ...
                {(locationPermission === 'denied' || locationPermission === 'prompt') && (<div className="absolute inset-0 z-10 flex items-center justify-center transition-all duration-500 ease-in-out" style={{
                borderRadius: '1rem', // Match parent container rounding
                overflow: 'hidden'
            }}>
                    <Button onClick={handleLocationPermission} className="w-full h-full backdrop-blur-md border-0 shadow-xl transition-all duration-300 hover:scale-[1.02] hover:backdrop-blur-lg" style={{
                ...
                padding: '12px 24px' // Match button padding
                      ...
                      <div className="flex items-center justify-center gap-3">
                        {weatherLoading ? (<Loader2 className="w-5 h-5 animate-spin"/>) : (<Sun className="w-5 h-5" style={{
                ...
                <div className={`p-6 space-y-4 transition-opacity duration-300 ${(locationPermission === 'denied' || locationPermission === 'prompt') ? 'opacity-0 invisible' : 'opacity-100 visible'}`}>
                  {/* Location Header */}
                  <div className="flex items-center justify-center gap-2">
                    <MapPin className="w-5 h-5" style={{ color: adaptiveTheme.accentColor }}/>
                    <span className="text-lg font-semibold tracking-wide" style={{ color: adaptiveTheme.textColor }}>
                  ...
                  <div className="flex items-center justify-center gap-4">
                    {/* Weather Icon */}
                    {weather && (<div className="flex items-center justify-center">
                        {weatherLoading ? (<Loader2 className="w-12 h-12 animate-spin" style={{ color: adaptiveTheme.accentColor }}/>) : (<div className="relative">
                            <img src={`/attached_assets/animated_weather_icons/${getWeatherIcon(weather.icon)}`} alt={weather.description} className="w-16 h-16 object-contain drop-shadow-lg" style={{
                    ...
                    <div className="flex flex-col items-center">
                      <span className="text-2xl font-bold tracking-tight" style={{ color: adaptiveTheme.textColor }}>
                        {weather ? `${Math.round(weather.temperature)}°F` : "Loading..."}
                      </span>
                      <span className="text-sm font-medium opacity-80 capitalize" style={{ color: adaptiveTheme.textColor }}>
          ...
          <div className={`relative transition-all duration-500 ease-in-out transform map-container ${isFullscreen
            ? 'fixed z-[9999] rounded-none'
            : 'w-full rounded-xl shadow-2xl border-2'}`} style={{
            ...
            {isLoading && (<div className="absolute inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-20">
                <div className="text-center">
            ...
            {/* Map Element */}
            <div ref={mapRef} className="w-full h-full transition-all duration-500 ease-in-out" style={{
            backgroundColor: isDarkMode ? "#1f2937" : "#f9fafb",
            overflow: "hidden"
        }}/>

            {/* Expand/Close Button - Always in top-left of map */}
            <Button onClick={toggleFullscreen} size="sm" className={`absolute ${isFullscreen ? 'top-8 left-8' : 'top-4 left-4'} z-[10000] border-0 shadow-lg transition-all duration-300 hover:scale-105 ${isFullscreen
            ...
            <div className={`absolute top-4 right-4 flex flex-col gap-2 z-10 transition-all duration-500 ease-in-out ${isFullscreen ? 'scale-110' : 'scale-100'}`}>
          ...
          <div className={`w-full max-w-7xl mx-auto mb-8 mt-8 transition-all duration-500 ease-in-out ${isFullscreen ? 'opacity-0 invisible' : 'opacity-100 visible'}`}>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                
                {/* Live Statistics Section - Redesigned */}
                <Card className="backdrop-blur-xl shadow-2xl border-2 transition-all duration-300 hover:shadow-3xl hover:scale-[1.02] stats-container" style={{
            ...
            boxShadow: `0 0 20px ${colors.primary}20`
                  ...
                  <CardHeader className="text-center pb-4">
                    <CardTitle className="text-2xl lg:text-3xl font-black tracking-wide flex items-center justify-center gap-3" style={{
            ...
            textShadow: isDarkMode ? `0 0 10px ${colors.primary}40` : 'none'
                  ...
                  <CardContent className="py-2">
                    <div className="grid grid-cols-1 gap-4">
                      ...
                      {/* Active Listeners */}
                      <div className="text-center p-6 rounded-2xl transition-all duration-300 hover:scale-[1.03]" style={{
            background: isDarkMode
            ...
            boxShadow: `0 8px 32px ${colors.primary}15`
                        ...
                        {statsLoading ? (<div className="animate-pulse text-5xl lg:text-6xl font-black" style={{ color: colors.primary }}>
                            --
                          </div>) : (<AnimatedCounter value={liveStats?.activeListeners || 42} className="text-5xl lg:text-6xl font-black" style={{
                color: colors.primary,
            ...
            boxShadow: `0 8px 32px ${colors.secondary}15`
                        ...
                        {statsLoading ? (<div className="animate-pulse text-5xl lg:text-6xl font-black" style={{ color: colors.secondary }}>
                            --
                          </div>) : (<AnimatedCounter value={liveStats?.countries || 15} className="text-5xl lg:text-6xl font-black" style={{
            ...
            boxShadow: `0 8px 32px ${colors.accent}15`
                        ...
                        <div className="flex items-center justify-center gap-2 mb-3">
                          <Activity className="w-6 h-6" style={{ color: colors.accent }}/>
                          <div className={`text-sm font-bold tracking-wide ${isDarkMode ? "text-gray-200" : "text-gray-800"}`}>
                            TOTAL LISTENERS
                          </div>
                        </div>
                        {statsLoading ? (<div className="animate-pulse text-5xl lg:text-6xl font-black" style={{ color: colors.accent }}>
                            ----
                          </div>) : (<AnimatedCounter value={liveStats?.totalListeners || 1247} className="text-5xl lg:text-6xl font-black" style={{
                color: colors.accent,
            ...
            { name: "Sydney, Australia", listeners: 28 }
        ].map((location, index) => (<div key={index} className="p-4 rounded-2xl transition-all duration-300 hover:scale-[1.03]" style={{
                background: isDarkMode
                ...
                boxShadow: `0 4px 20px ${colors.primary}10`
                          ...
                          <div className="flex items-center justify-center">
                            <div className="flex items-center space-x-3 flex-1 justify-center">
                              <div className="w-4 h-4 rounded-full animate-pulse flex-shrink-0" style={{
                backgroundColor: colors.primary,
                boxShadow: `0 0 15px ${colors.primary}90, 0 0 30px ${colors.primary}50`
            }}/>
                              <span className="font-black text-lg text-center" style={{
                ...
                textShadow: isDarkMode ? `0 2px 4px rgba(0,0,0,0.6)` : 'none'
                            ...
                            <div className="ml-4">
                              <AnimatedCounter value={location.listeners} className="text-xl font-black px-4 py-2 rounded-full" style={{
