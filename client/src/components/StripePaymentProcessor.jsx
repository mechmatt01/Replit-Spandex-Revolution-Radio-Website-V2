    ...
    return (<form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <Input id="name" value={customerInfo.name} onChange={(e) => setCustomerInfo((prev) => ({ ...prev, name: e.target.value }))} placeholder="John Doe" required className="bg-dark-bg/50 text-white placeholder-gray-400/60 focus:ring-2 focus:ring-metal-orange focus:ring-opacity-50"/>
        </div>
        <div>
          <Label htmlFor="email" className="text-gray-300 font-semibold">
            Email Address *
          </Label>
          <Input id="email" type="email" value={customerInfo.email} onChange={(e) => setCustomerInfo((prev) => ({ ...prev, email: e.target.value }))} placeholder="john@example.com" required className="bg-dark-bg/50 text-white placeholder-gray-400/60 focus:ring-2 focus:ring-metal-orange focus:ring-opacity-50"/>
        </div>
      </div>
      ...
      <Button type="submit" disabled={!stripe || processing} className="w-full bg-metal-orange hover:bg-orange-600 text-white px-6 py-3 rounded-full font-bold transition-all duration-300">
...
export default function StripePaymentProcessor() {
    ...
    if (showPaymentForm && selectedTier) {
        ...
        <Card className="bg-dark-bg/50 hover:bg-dark-bg/70 transition-all duration-300">
          <CardContent className="p-8">
            ...
            <Button variant="ghost" onClick={() => setShowPaymentForm(false)} className="w-full mt-4 text-gray-400 hover:text-white">
              Back to Plans
            </Button>
          </CardContent>
        </Card>
    ...
    return (<div className="w-full">
      ...
      <div className="md:hidden w-full">
        ...
        {subscriptionTiers.map((tier) => (<div key={`mobile-${tier.id}`} className="relative">
          {/* Title Above Box */}
          <h3 className="font-black text-white mb-4 text-center" style={{
                fontSize: tier.name === "LEGEND" ? "1.75rem" : "1.25rem" // 28px for LEGEND (20px + 8px), 20px for others
            }}>
            {tier.name}
          </h3>
          
          <div className="bg-transparent transition-all duration-300 relative rounded-lg flex flex-col" style={{
            ...
            {tier.popular && (<div className="absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2" style={{ top: "0" }}>
            ...
            <div className="p-6 flex flex-col h-full justify-between">
              <div>
                <div className="text-center mb-6">
                  ...
                  <div className="flex justify-center mb-4">
                    <img src={tier.icon} alt={`${tier.name} package icon`} className={`object-contain ${tier.id === "rebel" ? "w-20 h-20" : "w-16 h-16"}`}/>
                  </div>
                ...
                <ul className="space-y-4 mb-12 flex-grow">
                  {tier.features.map((feature, index) => (<li key={index} className="flex items-start justify-center">
              ...
              <button onClick={() => handleTierSelect(tier)} className="w-full font-bold rounded-full transition-all duration-300 text-white transform hover:scale-105 hover:shadow-2xl py-3 text-base" style={{
                fontSize: "16px",
      ...
      <div className="hidden md:block relative mx-auto" style={{
            width: "100%",
            maxWidth: "1000px",
            height: "600px"
        }}>
        {subscriptionTiers.map((tier, index) => (<div key={`desktop-${tier.id}`} className="absolute transition-all duration-300" style={{
            ...
            {tier.popular && (<div className="absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2" style={{
                    background: "linear-gradient(135deg, #ff6b35, #f7931e)",
                    color: "black",
                    whiteSpace: "nowrap",
                    fontSize: "11px",
            ...
            <div className="rounded-lg flex flex-col transition-all duration-300 relative overflow-hidden" style={{
