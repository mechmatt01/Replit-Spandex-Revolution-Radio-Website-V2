  ...
  const handleSendMessage = async () => {
    ...
    try {
      await measureAsyncOperation('live_chat_send_message', async () => {
        ...
        // Add visual feedback - create a temporary message that swooshes away
        ...
        // Remove temporary message after animation
        setTimeout(() => {
              ...
              <div className="flex-1 overflow-y-auto px-4" ref={scrollAreaRef} style={{ minHeight: '300px', maxHeight: '400px' }}>
                <div className="space-y-3 py-2">
                  {messages.length === 0 ? (
                    ...
                    messages.map((msg) => (
                      <div key={msg.id} className={`flex gap-2 chat-message ${msg.id.startsWith('temp-') ? 'swooshing' : ''}`}>
              ...
              <div className="flex-shrink-0 p-4 border-t" style={{ borderColor: colors.primary + '20' }}>
                <form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="flex gap-2">
                  ...
                  <Button
                    type="submit"
                    size="sm"
                    className="px-3 focus:outline-none focus:ring-0 transition-all duration-200"
                    style={{
                      backgroundColor: colors.primary,
                      color: colors.primaryText || "white",
