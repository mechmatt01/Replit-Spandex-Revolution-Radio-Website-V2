...
export default function ShoppingCartComponent({ cart, onUpdateQuantity, onRemoveItem, onCheckout }: ShoppingCartProps) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('cartItemAdded' as any, handleCartAnimation);
  }, []);
    ...
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute top-full right-0 mt-2 w-80 z-50">
          <Card style={{ backgroundColor: colors.card, borderColor: colors.primary }}>
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-lg" style={{ color: colors.text }}>Shopping Cart</h3>
                    ...
                    {cart.map((item) => (
                      <div key={item.variantId} className="flex items-center space-x-3">
                        ...
                        <div className="flex-1">
                          <h4 className="text-sm font-semibold" style={{ color: colors.text }}>{item.title}</h4>
                          <p className="text-sm" style={{ color: colors.textMuted }}>${item.price}</p>
                        </div>
                        <div className="flex items-center space-x-1">
                          ...
                          <span className="text-sm mx-2" style={{ color: colors.text }}>{item.quantity}</span>
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => onUpdateQuantity(item.variantId, item.quantity + 1)}
                  ...
                  <div className="border-t pt-3" style={{ borderColor: colors.primary }}>
                    <div className="space-y-1 text-sm">
                      ...
                      <div className="flex justify-between font-bold">
                        ...
                        <span style={{ color: colors.text }}>${total.toFixed(2)}</span>
                      </div>
                    </div>
                    <Button
                      onClick={onCheckout}
