...
export default function ShopifyEcommerce() {
    ...
    return (<section id="merch" className="py-20 transition-colors duration-300" style={{ backgroundColor: colors.background }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="font-orbitron font-black text-3xl md:text-4xl mb-4" style={{
            color: currentTheme === 'light-mode' ? '#000000' : colors.text
        }}>
            OFFICIAL MERCH STORE
          </h2>
          <p className="text-lg font-semibold" style={{ color: colors.textMuted }}>
        ...
        {/* Cart Summary */}
                      {(cart || []).length > 0 && (<div className="mb-8">
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <span className="font-black" style={{ color: currentTheme === 'light-mode' ? '#000000' : '#ffffff' }}>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <Button onClick={handleCheckout} className="bg-metal-orange hover:bg-orange-600 text-white px-6 py-2 rounded-full font-bold">
                      Checkout
                    </Button>
                  </div>
                </div>
        ...
        <div className="mb-12 text-center">
          ...
          <div className="w-full flex justify-center">
            ...
            .map((product) => (<ProductCard key={product.id} product={product} onAddToCart={(variant, quantity) => addToCart(product, variant, quantity)} onViewDetails={() => setSelectedProduct(product)}/>))}
            </div>
          </div>
        </div>

        {/* Product Modal */}
        {selectedProduct && (<ProductModal product={selectedProduct} onClose={() => setSelectedProduct(null)} onAddToCart={(variant, quantity) => addToCart(selectedProduct, variant, quantity)}/>)}
      </div>
    </section>);
}
function ProductCard({ product, onAddToCart, onViewDetails, }) {
    ...
    const { getColors, currentTheme } = useTheme();
    const colors = getColors();
    return (<Card className="transition-all duration-300 group h-full flex flex-col" style={{
      ...
      <CardContent className="p-4 flex flex-col h-full">
        <div className="relative mb-4">
          ...
          {product.featured && (<Badge className="absolute top-2 left-2" style={{ backgroundColor: colors.primary, color: "white" }}>
              Featured
            </Badge>)}
          {product.compareAtPrice && (<Badge className="absolute top-2 left-2 w-auto" style={{
          ...
          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-2">
            <Button size="sm" variant="secondary" onClick={onViewDetails} className="bg-white/90 text-black hover:bg-white">
        ...
        <h4 className="font-black mb-2" style={{ color: currentTheme === 'light-mode' ? '#000000' : '#ffffff' }}>
          {product.title}
        </h4>
        <p className="text-sm font-semibold line-clamp-2 flex-grow" style={{ color: colors.textMuted }}>
          {product.description}
        </p>

        <div className="flex flex-col items-center mb-3 mt-3">
          <div className="flex items-center">
            {[...Array(5)].map((_, i) => (<Star key={i} className={`h-4 w-4 ${i < Math.floor(product.rating)
          ...
          <div className="mt-2 mb-3">
            ...
            {product.compareAtPrice && (<span className="line-through ml-2" style={{
                color: currentTheme === 'light-mode' ? '#6b7280' : '#9ca3af'
            }}>
                ${product.compareAtPrice}
              </span>)}
          </div>
          {!product.inStock && (<Badge variant="secondary" className="bg-gray-600 text-white mt-2">
              Out of Stock
            </Badge>)}
        </div>

        <div className="mt-auto">
          {(product.variants || []).length > 1 && (<div className="relative mb-3">
              <select value={selectedVariant.id} onChange={(e) => setSelectedVariant(product.variants?.find((v) => v.id === e.target.value))} className="w-full p-2 rounded appearance-none pr-8 text-center" style={{
                backgroundColor: currentTheme === 'light-mode' ? 'rgba(255, 255, 255, 0.9)' : 'rgba(30, 30, 30, 0.5)',
          ...
          <Button onClick={() => console.log('Buy now:', product, selectedVariant)} disabled={!product.inStock || !selectedVariant.available} className="w-full font-bold transition-all duration-300" style={{
      ...
      <Card className="bg-dark-bg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <CardContent className="p-6">
          <div className="flex justify-between items-start mb-6">
            <h3 className="font-black text-2xl" style={{ color: currentTheme === 'light-mode' ? '#000000' : '#ffffff' }}>
              {product.title}
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <p className="font-semibold mb-4" style={{ color: colors.textMuted }}>
                {product.description}
              </p>

              <div className="flex items-center mb-4">
                <div className="flex items-center">
                  {[...Array(5)].map((_, i) => (<Star key={i} className={`h-5 w-5 ${i < Math.floor(product.rating)
                ? "text-yellow-400 fill-current"
                : "text-gray-600"}`}/>))}
                </div>
            ...
            color: '#9ca3af'
              ...
              {(product.variants || []).length > 1 && (<div className="mb-4">
                  ...
                  <select value={selectedVariant.id} onChange={(e) => setSelectedVariant(product.variants?.find((v) => v.id === e.target.value))} className="w-full p-2 bg-dark-bg text-white placeholder-gray-500 rounded text-center" style={{ textAlign: 'center' }}>
                    {(product.variants || []).map((variant) => (<option key={variant.id} value={variant.id} disabled={!variant.available}>
                        {variant.title} {!variant.available && "(Out of Stock)"}
                      </option>))}
                  </select>
              ...
              <div className="mb-6">
                ...
                <input type="number" min="1" max="10" value={quantity} onChange={(e) => setQuantity(parseInt(e.target.value))} className="w-20 p-2 bg-dark-bg/50 text-white rounded"/>
              </div>

              <Button onClick={() => {
            console.log('Buy now:', product, selectedVariant);
            onClose();
        }} disabled={!product.inStock || !selectedVariant.available} className="w-full bg-metal-orange hover:bg-orange-600 text-white font-bold">
                <ShoppingCart className="mr-2 h-4 w-4"/>
