...
export default function StripePaymentProcessor() {
  ...
  const [subscriptionTiers, setSubscriptionTiers] = useState<SubscriptionTier[]>(defaultSubscriptionTiers);
    ...
    const fetchSubscriptionTiers = async () => {
      try {
        ...
        if (data.success && data.products) {
          // Map API products to subscription tiers
          const tiers = data.products.map((product: any) => ({
            ...
            gradientStart: product.name === 'Rebel' ? "#B56BFF" : 
                          product.name === 'Legend' ? "#E520C6" : "#FF50C3",
            gradientEnd: product.name === 'Rebel' ? "#FF50C3" : 
                        product.name === 'Legend' ? "#F4654F" : "#B66BFF",
        ...
        // Keep default tiers if API fails
      } finally {
        setLoading(false);
      }
    };
