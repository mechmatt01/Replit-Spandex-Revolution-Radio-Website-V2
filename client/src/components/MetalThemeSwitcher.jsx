    ...
    return (<div className="flex items-center gap-2">
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <Button variant="ghost" size="sm" className="flex items-center gap-2 hover:bg-opacity-20 theme-button-border" style={{
            color: currentTheme === 'light-mode' ? '#000000' : colors.text,
            backgroundColor: "transparent",
        }}>
            <Palette className="w-4 h-4" style={{ color: colors.primary }}/>
            <span className="hidden sm:inline text-sm font-medium" style={{
        ...
        <PopoverContent className="w-80 p-0 border-0 max-h-[80vh] overflow-hidden" style={{
            backgroundColor: colors.surface,
            border: `1px solid ${colors.border}`,
        }}>
          <Card className="border-0 shadow-xl" style={{ backgroundColor: colors.surface }}>
            <CardHeader className="pb-3">
              <CardTitle className="text-lg font-black flex items-center gap-2" style={{ color: colors.text }}>
            ...
            return (<Button key={themeKey} onClick={() => handleThemeClick(themeKey, themeConfig)} variant="ghost" className={`w-full justify-start p-3 h-auto transition-all duration-200 ${isActive ? "ring-2" : ""} ${isLocked ? "opacity-75" : ""}`} style={{
                      ...
                      <div className="flex-1 text-left min-w-0">
                        <div className="flex items-center gap-2 justify-between">
                          <div className="flex items-center gap-2 flex-1">
                            <span className="font-semibold text-sm" style={{ color: isActive ? 'white' : themeColors.primary }}>
                              {themeConfig.name}
                            </span>
                            {isPremium && !isActive && (<Star className="w-4 h-4 flex-shrink-0" style={{
                        color: "#ffd700",
                        fill: "#ffd700",
                        filter: isLocked
                            ...
                            {isActive && (<Badge variant="secondary" className="text-xs px-2 py-0" style={{
                        ...
                        <p className="text-xs mt-1 break-words leading-relaxed whitespace-pre-wrap overflow-hidden" style={{
                    ...
                    overflowWrap: "break-word",
                        ...
                        <div className="flex gap-1 mt-2">
                          {themeKey === "glassmorphism-premium" ? (<div className="flex gap-1">
            ...
            border: "1px solid rgba(255, 255, 255, 0.2)",
                  ...
                  <div className="mt-4 space-y-4">
                    ...
                    <div className="flex gap-2 mt-4">
                      <Button onClick={() => setShowPremiumDialog(false)} variant="ghost" className="flex-1 text-white border border-primary/30 hover:bg-white/20 font-semibold px-4 py-2 text-sm" style={{
            ...
            borderRadius: "6px",
                      ...
                      <Button onClick={scrollToSubscription} className="flex-1 font-bold px-4 py-2 text-sm text-white" style={{
            ...
            setShowPremiumDialog(false);
            setIsOpen(false);
        }} className="w-full font-semibold px-4 py-2 text-xs text-yellow-300" style={{
