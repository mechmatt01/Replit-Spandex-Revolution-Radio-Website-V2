...
import { Globe, MapPin, Users, Maximize2, Loader2, Cloud, Sun, CloudRain, CloudSnow, CloudDrizzle, CloudLightning, X, } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import { useTheme } from "@/contexts/ThemeContext";
// Generate realistic active listeners with Firebase-style data
const generateActiveListeners = () => [
    {
        id: "listener_001",
...
const GoogleMapWithListeners = ({ listeners, colors, isDarkMode, themeName, onListenerClick, selectedListener, apiKey, userLocation, setSelectedListener, setIsUserLocationSelected, }) => {
...
export default function InteractiveListenerMap() {
    ...
    return (<section id="map" className={`py-20 ${isDarkMode ? "bg-black" : "bg-white"} ${isFullscreen ? "fixed inset-0 z-40" : ""} transition-all duration-500 ease-in-out`} style={{
            ...(isFullscreen && {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          {!locationPermissionDenied && (<>
              {weatherLoading && userLocation && (<div className="mb-4">
              ...
              {weather && (<div className="mb-4">
                  ...
                  <div className="flex items-center justify-center gap-2">
                    <WeatherIcon iconCode={weather.icon} className={`w-6 h-6 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}/>
              ...
              {!weather && !weatherLoading && userLocation && (<div className="mb-4">
                  <p className={`text-lg font-semibold mb-2 ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}>
          ...
          {locationPermissionDenied && (<div className="mb-4">
        ...
        {/* Fullscreen Overlay */}
        {isFullscreen && (<div className="fixed inset-0 z-[9999] bg-black animate-fadeIn">
            <div className="h-full flex flex-col">
              ...
              <div className="flex-1 relative overflow-hidden">
                {!isLoading && apiKey && (<GoogleMapWithListeners listeners={activeListeners} colors={colors} isDarkMode={true} themeName={currentTheme} onListenerClick={setSelectedListener} selectedListener={selectedListener} apiKey={apiKey} userLocation={userLocation || undefined} setSelectedListener={setSelectedListener} setIsUserLocationSelected={setIsUserLocationSelected}/>)}
              </div>
            </div>
          </div>)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <Card className={`${isDarkMode ? "bg-gray-900/50 hover:bg-gray-900/70" : "bg-gray-100/50 hover:bg-gray-100/70"} transition-all duration-300 border-2`} style={{ borderColor: colors.primary }}>
              <CardContent className="p-6">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex gap-2">
                    <Button variant="ghost" size="sm" onClick={toggleFullscreen} className={`${isDarkMode ? "text-white hover:bg-white/10" : "text-black hover:bg-black/10"}`}>
                      <Maximize2 className="w-4 h-4"/>
                    </Button>
                  </div>
                </div>

                <div className={`relative ${isDarkMode ? "bg-gray-800" : "bg-gray-200"} rounded-lg h-96 overflow-hidden transition-all duration-500 ease-in-out`}>
                  {/* Loading State */}
                  {isLoading && (<div className="absolute inset-0 flex items-center justify-center z-10">
                      <div className="text-center">
                        ...
                        <div className="flex justify-center space-x-2">
                          {[0, 1, 2].map((i) => (<div key={i} className="w-3 h-3 rounded-full animate-pulse" style={{
                ...
                return null;
                      ...
                      {apiKey ? (<GoogleMapWithListeners listeners={activeListeners} colors={colors} isDarkMode={isDarkMode} themeName={currentTheme} onListenerClick={setSelectedListener} selectedListener={selectedListener} apiKey={apiKey} userLocation={userLocation || undefined} setSelectedListener={setSelectedListener} setIsUserLocationSelected={setIsUserLocationSelected}/>) : (<div className="flex items-center justify-center h-full">
                          <div className="text-center">
                            <Globe className={`${isDarkMode ? "text-gray-600" : "text-gray-400"} h-16 w-16 mx-auto mb-4 opacity-50`}/>
                            <p className={`font-semibold ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
                  ...
                  {selectedListener && (<div className="absolute bottom-4 left-4 p-4 rounded-lg shadow-lg border z-20" style={{
                backgroundColor: isDarkMode ? "#1f2937" : "#ffffff",
                borderColor: colors.primary,
            }}>
                      <button onClick={() => setSelectedListener(null)} className={`absolute top-2 right-2 ${isDarkMode ? "text-gray-400 hover:text-white" : "text-gray-600 hover:text-black"} text-lg`}>
                        ×
                      </button>
          ...
          {!isFullscreen && (<div className="space-y-6">
              <Card className={`${isDarkMode ? "bg-gray-900/50 hover:bg-gray-900/70" : "bg-gray-100/50 hover:bg-gray-100/70"} transition-all duration-300 border-2`} style={{ borderColor: colors.primary }}>
                <CardContent className="p-6">
                  <h3 className="font-black text-xl mb-3" style={{ color: colors.primary }}>
                    Live Statistics
                  </h3>
              ...
              {!locationPermissionDenied && (<Card className={`${isDarkMode ? "bg-gray-900/50 hover:bg-gray-900/70" : "bg-gray-100/50 hover:bg-gray-100/70"} transition-all duration-300 border-2`} style={{ borderColor: colors.primary }}>
                  <CardContent className="p-6">
                    <h3 className="font-black text-xl mb-3" style={{ color: colors.primary }}>
                      Active Locations
                    </h3>
                    ...
                    .map((listener) => (<div key={listener.id} className={`flex items-center justify-between p-2 rounded transition-colors duration-200 cursor-pointer ${selectedListener?.id === listener.id
                        ? "bg-opacity-20"
                        : "hover:bg-opacity-10"}`} style={{
                        ...
                        setIsUserLocationSelected(false);
                            ...
                            <div className="flex items-center">
                              ...
                              <div>
                                <div className={`font-semibold text-sm ${isDarkMode ? "text-white" : "text-black"}`}>
                                  {listener.city}
                                </div>
