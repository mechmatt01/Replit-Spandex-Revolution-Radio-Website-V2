...
export default function SubscriptionCarousel() {
    ...
    return (<div className="relative w-full max-w-6xl mx-auto px-4">
      {/* 3D Carousel Container - Responsive height with device compatibility */}
      <div className="relative perspective-1000 subscription-carousel-container overflow-visible py-2 sm:py-4 md:py-6 lg:py-8" style={{
            minHeight: '700px',
            height: 'auto'
        }}>
        {/* Navigation Buttons */}
        <button onClick={handlePrevious} className="absolute left-2 top-1/2 transform -translate-y-1/2 z-10 bg-black/50 hover:bg-black/70 rounded-full p-2 transition-colors" disabled={isAnimating} title="Previous subscription">
          <ChevronLeft className="w-8 h-8 text-white hover:scale-110 transition-transform drop-shadow-md"/>
        </button>

        <button onClick={handleNext} className="absolute right-2 top-1/2 transform -translate-y-1/2 z-10 bg-black/50 hover:bg-black/70 rounded-full p-2 transition-colors" disabled={isAnimating} title="Next subscription">
        ...
        <div className="flex items-center justify-center h-full relative">
          <div className="relative w-full max-w-md" style={{
            ...
            <div className="absolute inset-4 rounded-3xl overflow-hidden">
              <div className="absolute inset-0 blur-2xl opacity-40" style={{
            ...
            padding: '5px', // Increased border width by 2.5px (from 2.5px to 5px)
              ...
              {/* Main Card */}
              <div className="relative backdrop-blur-xl p-6 overflow-hidden flex flex-col subscription-card" style={{
        ...
        }} onMouseEnter={() => setHoveredTier(currentTier.id)} onMouseLeave={() => setHoveredTier(null)}>


              {/* Package Icon with Animation */}
              <div className="flex justify-center mb-2 sm:mb-4">
                <div className="relative p-2 sm:p-3 rounded-full" style={{
            background: `linear-gradient(135deg, ${currentTier.gradientStart}20, ${currentTier.gradientEnd}20)`,
        }}>
                  <img src={currentTier.icon} alt={`${currentTier.name} icon`} className={`w-16 h-16 sm:w-20 sm:h-20 object-contain animate-float ${currentIndex === 1 ? 'float-delay-1' :
            ...
            color: currentTheme === 'light-mode' ? 'rgba(0, 0, 0, 0.6)' : '#9ca3af'
                ...
                {currentTier.popular && (<div className="mt-4">
            ...
            color: currentTheme === 'light-mode' ? '#000000' : '#ffffff'
                  ...
                  <span className="ml-2 text-sm sm:text-base" style={{
            color: currentTheme === 'light-mode' ? 'rgba(0, 0, 0, 0.6)' : '#9ca3af'
        }}>
                    /month
                  </span>
              ...
              <div className="flex-1 flex items-center justify-center py-2 sm:py-4">
                <div className="w-full max-w-xs">
                  {(currentTier.features || []).sort((a, b) => (a?.length || 0) - (b?.length || 0)).map((feature, index) => (<div key={index} className="flex items-center justify-center mb-2 sm:mb-3 transform transition-all duration-300" style={{
                transform: hoveredTier === currentTier.id ? "translateX(10px)" : "translateX(0)",
                transitionDelay: `${index * 50}ms`,
            }}>
                      <div className="flex items-center">
                        <div className="w-3 h-3 sm:w-4 sm:h-4 rounded-full flex items-center justify-center flex-shrink-0 mr-2 sm:mr-3" style={{
                background: `linear-gradient(135deg, ${currentTier.gradientStart}, ${currentTier.gradientEnd})`,
            }}>
                          <Check className="w-2 h-2 sm:w-2.5 sm:h-2.5 text-white"/>
                        </div>
                        <span className="text-xs sm:text-sm font-medium text-center" style={{
              ...
              <div className="mb-2 sm:mb-3">
                <div className="bg-white/5 rounded-lg p-2 sm:p-3 max-w-xs mx-auto">
                  <h4 className="text-xs font-bold mb-1 sm:mb-2 text-center" style={{
            color: currentTheme === 'light-mode' ? 'rgba(0, 0, 0, 0.6)' : '#9ca3af'
        }}>
                    EXCLUSIVE PERKS
                  </h4>
                  <div className="space-y-0.5 sm:space-y-1">
                    {(currentTier.perks || []).map((perk, index) => (<div key={index} className="text-xs text-center leading-tight" style={{
                color: currentTheme === 'light-mode' ? 'rgba(0, 0, 0, 0.7)' : '#d1d5db'
            }}>
                        {perk}
                      </div>))}
              ...
              <div className="mt-auto mb-3 sm:mb-4">
                <Button className="w-full py-3 sm:py-4 text-sm sm:text-base font-bold rounded-xl transform transition-all duration-300 hover:scale-105" style={{
            background: `linear-gradient(135deg, ${currentTier.gradientStart}, ${currentTier.gradientEnd})`,
      ...
      <div className="flex justify-center items-center space-x-6 mt-6">
        {(subscriptionTiers || []).map((tier, index) => (<button key={tier.id} onClick={() => handleSelectTier(index)} className={cn("relative transition-all duration-500 ease-in-out p-3 rounded-full", index === currentIndex
                ? "scale-110"
                : "opacity-40 hover:opacity-60 scale-90")}>
            {/* Smooth background glow */}
            <div className={cn("absolute inset-0 rounded-full transition-all duration-500", index === currentIndex ? "opacity-100" : "opacity-0")} style={{
                background: `radial-gradient(circle, ${tier.gradientStart}20, transparent 70%)`,
                filter: "blur(12px)",
            }}/>

            {/* Icon container */}
            <div className={cn("relative rounded-full transition-all duration-500", index === currentIndex ? "bg-black/20" : "bg-transparent")} style={{
                padding: "0.5rem",
            }}>
              <img src={tier.icon} alt={`${tier.name} icon`} className={cn("object-contain transition-all duration-500", index === currentIndex ? "w-8 h-8" : "w-6 h-6")} style={{
            ...
            {index === currentIndex && (<div className="absolute inset-0 rounded-full" style={{
