...
const Toast = React.forwardRef(({ className, variant, ...props }, ref) => {
    ...
    return (<ToastPrimitives.Root ref={ref} className={cn(toastVariants({ variant }), className)} style={getToastStyles()} {...props}/>);
});
Toast.displayName = ToastPrimitives.Root.displayName;
const ToastAction = React.forwardRef(({ className, ...props }, ref) => {
    const { colors } = useTheme();
    return (<ToastPrimitives.Action ref={ref} className={cn("inline-flex h-8 shrink-0 items-center justify-center rounded-md border px-3 text-sm font-medium transition-colors hover:bg-secondary disabled:pointer-events-none disabled:opacity-50", className)} style={{
            backgroundColor: colors.background,
            borderColor: colors.primary,
            color: colors.text,
        }} {...props}/>);
...
const ToastTitle = React.forwardRef(({ className, ...props }, ref) => {
    const { colors } = useTheme();
    return (<ToastPrimitives.Title ref={ref} className={cn("text-sm font-semibold", className)} style={{ color: colors.text }} {...props}/>);
});
ToastTitle.displayName = ToastPrimitives.Title.displayName;
const ToastDescription = React.forwardRef(({ className, ...props }, ref) => {
    const { colors } = useTheme();
    return (<ToastPrimitives.Description ref={ref} className={cn("text-sm opacity-90", className)} style={{ color: colors.text }} {...props}/>);
});
