...
export default function StickyPlayer() {
    ...
    return (<div className={`fixed bottom-2 left-4 backdrop-blur-md z-50 transition-all duration-700 rounded-2xl shadow-2xl border-0 floating-player-no-focus ${isVisible
            ...
            color: currentTrack?.artwork && currentTrack.artwork !== 'advertisement'
                ...
                : colors.text,
            border: 'none'
        }} role="region" aria-label="Floating audio player" aria-live="polite">
      <div className="w-full px-3 py-2 relative">
        {/* Compact floating player layout */}
        <div className="flex items-center justify-between mb-2">
          {/* Album Art */}
          <InteractiveAlbumArt artwork={isAdPlaying && adInfo.artwork ? adInfo.artwork : currentTrack.artwork} title={currentTrack.title} artist={currentTrack.artist} size="sm" className="w-12 h-12" isAd={isAdPlaying}/>

          {/* Track Info with 60% width of player box */}
          <div className="min-w-0 ml-3 focus-safe-area" style={{ width: "60%" }}>
            {/* Ad Detection Badge */}
            {isAdPlaying && (<div className="mb-1 flex justify-start">
                <div className="inline-flex items-center px-1.5 py-0.5 rounded-full text-xs font-bold text-white animate-pulse" style={{
                background: `linear-gradient(45deg, #ff4444, #cc0000)`,
                boxShadow: `0 1px 4px #ff444460`,
            }}>
                  <span className="mr-1 text-xs">📢</span>
                  AD
                  {adInfo.company && (<span className="ml-1 opacity-80 text-xs">
                      • {adInfo.company}
                    </span>)}
                </div>
              </div>)}
        ...
        }} maxWidth="100%" isFloating={true} backgroundColor="transparent" alignment="left"/>
            </div>
            
            {/* Artist Information */}
            {currentTrack.artist &&
            currentTrack.artist !== currentTrack.title &&
            currentTrack.artist !== stationName && (<div className="text-xs truncate mt-0.5" style={{
            ...
            <div className="flex items-center justify-between mt-1">
              <div className="flex items-center space-x-1">
                {/* LIVE/AD indicator */}
                <div className={`w-2 h-2 rounded-full animate-pulse ${isAdPlaying ? 'bg-red-600' : 'bg-red-500'}`}></div>
                <span className={`text-xs font-medium ${isAdPlaying ? 'text-red-600' : 'text-red-500'}`}>
                  {isAdPlaying ? 'AD' : 'LIVE'}
                </span>
              </div>
            ...
            background: `linear-gradient(45deg, ${colors.primary}, ${colors.secondary})`,
                    ...
                    <input type="range" min="0" max="100" value={(isMuted ? 0 : volume) * 100} onChange={handleVolumeChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" aria-label="Volume control" title="Adjust volume"/>
                  </div>
          ...
          <div className="relative">
            {isLoading && (<div className="absolute inset-0 flex items-center justify-center">
                <div className="w-7 h-7 border-2 border-t-transparent rounded-full animate-spin" style={{
            ...
            <Button onClick={togglePlayback} className="text-white w-10 h-10 rounded-full   flex items-center justify-center transition-all duration-300 hover:scale-110 shadow-lg hover:shadow-xl border-0" style={{
            background: `linear-gradient(45deg, ${colors.primary}, ${colors.secondary})`,
