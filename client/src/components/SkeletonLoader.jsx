...
export default function SkeletonLoader({ width = '100%', height = '20px', className = '', variant = 'rectangular', lines = 1 }) {
    ...
    if (variant === 'circular') {
        return (<div className={`${baseClasses} rounded-full`} style={{
                width,
                height,
                ...shimmerEffect
            }}/>);
    ...
    if (variant === 'text') {
        ...
        {Array.from({ length: lines || 0 }).map((_, index) => (<div key={index} className={`${baseClasses} rounded`} style={{
    ...
    return (<div className={`${baseClasses} rounded`} style={{
            width,
            height,
            ...shimmerEffect
        }}/>);
