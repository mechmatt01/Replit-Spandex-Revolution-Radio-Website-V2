...
export default function RadioCoPlayer() {
    const { currentTrack, isPlaying, isLoading, error, togglePlayback, volume, setVolume, isMuted, toggleMute, changeStation, currentStation, } = useRadio();
    ...
    return (<section className="bg-card/50 backdrop-blur-sm rounded-2xl p-6 shadow-lg" role="region" aria-label="Radio player controls">
      ...
      <div className="mb-6">
        <div className="relative" ref={stationDropdownRef}>
          <Button variant="outline" size="sm" onClick={() => setIsStationDropdownOpen(!isStationDropdownOpen)} className="bg-card/90 backdrop-blur-sm hover:bg-card/95 transition-all duration-200 text-xs px-3 py-1" style={{
          ...
          {isStationDropdownOpen && (<div className="absolute mt-1 left-1/2 transform -translate-x-1/2 max-h-60 overflow-y-auto bg-black/90 backdrop-blur-lg border shadow-xl z-20 scrollbar-thin" style={{
      ...
      <div className="flex justify-center mb-6 relative">
        <div className={`transition-all duration-1000 transform ${isTransitioning
          ...
          <InteractiveAlbumArt artwork={currentTrack.artwork} title={currentTrack.title} artist={currentTrack.artist} size="lg"/>
        </div>
      ...
      <div className="text-center mb-6">
        <div className={`transition-all duration-700 transform ${isTransitioning
            ? "opacity-0 scale-95 translate-y-2 blur-sm"
            : "opacity-100 scale-100 translate-y-0 blur-0"}`} style={{
            transformOrigin: "center",
          ...
          <div className="flex justify-center mb-2 w-full overflow-hidden">
            <div className={`w-full max-w-full ${isTransitioning ? "song-change-shimmer" : ""}`}>
              <ScrollingText text={currentTrack.title} className="font-black text-foreground whitespace-nowrap" style={{ fontSize: "32px", lineHeight: "1" }} maxWidth="100%" backgroundColor="hsl(var(--background))"/>
            </div>
          </div>
          {currentTrack.artist &&
            ...
            currentTrack.artist !==
                (selectedStation?.name || "95.5 The Beat") && (<p className="text-foreground font-black text-2xl mb-1 transition-opacity duration-500">
                {currentTrack.artist}
              </p>)}
          {currentTrack.album &&
            ...
            currentTrack.album !==
                (selectedStation?.name || "95.5 The Beat") && (<p className="text-muted-foreground text-sm font-medium mb-2 transition-opacity duration-500">
                {currentTrack.album}
              </p>)}
          {currentTrack.title !== "Live Stream" &&
            ...
            (selectedStation?.name || "95.5 The Beat") !==
                currentTrack.title && (<p className="text-foreground font-semibold text-lg transition-opacity duration-500">
                {selectedStation?.name || "95.5 The Beat"}
              </p>)}
        </div>
      </div>
      ...
      <div className="flex flex-col items-center justify-center space-y-4 relative">
        <div className="flex items-center justify-center w-full">
          <Button onClick={togglePlayback} disabled={isLoading} className="font-bold py-6 px-10 rounded-full transition-all duration-300 shadow-2xl hover:shadow-3xl transform hover:scale-110 disabled:opacity-50 disabled:transform-none text-xl flex items-center justify-center mx-auto relative" style={{
            ...
            {isLoading ? (<>
              ...
              </>) : isPlaying ? (<svg className="h-16 w-16" fill="currentColor" viewBox="0 0 24 24">
                <rect x="4" y="4" width="16" height="16" rx="4" strokeLinejoin="round" strokeLinecap="round"/>
              </svg>) : (<svg className="h-15 w-15" fill="currentColor" viewBox="0 0 24 24" style={{
