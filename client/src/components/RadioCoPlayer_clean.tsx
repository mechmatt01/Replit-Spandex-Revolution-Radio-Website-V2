    ...
    <section className="flex flex-col items-center justify-center space-y-8 px-8 py-12 text-center">
      ...
      <div className="relative" ref={dropdownRef}>
        <Button
          ...
          className="flex items-center justify-between px-6 py-3 rounded-xl transition-all duration-300 hover:scale-105 border-2 min-w-[280px]"
          style={{
        ...
        >
          <div className="flex items-center space-x-3">
            <div className="flex items-center justify-center w-8 h-8 rounded-full text-lg"
              ...
              {currentStation.icon}
              {/* Animated volume icon for currently selected station */}
              {isPlaying && (
            ...
            <div className="text-left">
              <div className="font-semibold text-foreground">{currentStation.name}</div>
              <div className="text-sm text-muted-foreground">{currentStation.frequency} • {currentStation.location}</div>
            </div>
          </div>
          <ChevronDown 
        ...
        {isDropdownOpen && (
          <div 
            className="absolute top-full left-0 right-0 mt-2 bg-background/95 backdrop-blur-md rounded-xl shadow-xl border z-50 overflow-hidden max-h-[300px] overflow-y-auto"
            style={{ 
          ...
          >
            {radioStations.map((station) => (
              <button
                ...
                className="w-full px-4 py-3 text-left hover:bg-white/10 transition-colors duration-200 flex items-center space-x-3 border-b border-white/10 last:border-b-0"
              >
                <div className="flex items-center justify-center w-8 h-8 rounded-full text-lg"
                  style={{ 
                    background: station.id === selectedStation ? `${colors.primary}50` : `${colors.primary}20`,
                    color: station.id === selectedStation ? colors.primary : 'inherit'
                  }}>
                  {station.icon}
                </div>
                <div className="flex-1">
                  <div className="font-medium text-white">{station.name}</div>
                  <div className="text-sm text-gray-400">{station.frequency} • {station.genre}</div>
