...
import { Settings, Calendar, ShoppingBag, Users, TrendingUp, Edit, Plus, X, Lock } from "lucide-react";
export default function AdminPanel({ onClose }) {
        ...
        <Card ref={modalRef} className={`w-full max-w-md relative ${isDarkMode ? "bg-gray-900" : "bg-white"}`}>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="admin-username" type="text" value={adminUsername} onChange={(e) => setAdminUsername(e.target.value)} placeholder="Username" className={`${isDarkMode ? "placeholder:text-gray-400" : "placeholder:text-gray-400"} placeholder:opacity-50`} disabled={isLoading}/>
            </div>
            <div className="space-y-2">
              <Label htmlFor="admin-password">Password</Label>
              <Input id="admin-password" type="password" value={adminPassword} onChange={(e) => setAdminPassword(e.target.value)} placeholder="Password" className={`${isDarkMode ? "placeholder:text-gray-400" : "placeholder:text-gray-400"} placeholder:opacity-50`} onKeyDown={(e) => e.key === "Enter" && !isLoading && handleAdminLogin()} disabled={isLoading}/>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={onClose} className="flex-1 transition-all duration-300 hover:bg-red-500/10 hover:border-red-500 hover:text-red-500" disabled={isLoading}>
                Cancel
              </Button>
              <Button onClick={handleAdminLogin} className="flex-1 transition-all duration-300 hover:scale-105" style={{ backgroundColor: colors.primary }} disabled={isLoading}>
    ...
    return (<div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className={`w-full max-w-6xl max-h-[90vh] overflow-y-auto ${isDarkMode ? "bg-gray-900" : "bg-white"} rounded-lg shadow-2xl`}>
        ...
        <div className="p-6">
        ...
        ].map((tab) => (<button key={tab.id} onClick={() => setActiveSection(tab.id)} className={`flex items-center space-x-2 px-4 py-2 rounded-md text-sm font-medium transition-all ${activeSection === tab.id
          ...
          {activeSection === "overview" && (<div className="space-y-6">
            ...
            ].map((activity, index) => (<div key={index} className="flex items-center space-x-3">
                ...
                <Card className={isDarkMode ? "bg-gray-800" : "bg-white"}>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 gap-3">
                      <Button variant="outline" className="h-auto p-3 flex flex-col items-center space-y-2" onClick={() => setActiveSection("schedule")}>
                        <Calendar className="w-5 h-5"/>
                        <span className="text-xs">Add Show</span>
                      </Button>
                      <Button variant="outline" className="h-auto p-3 flex flex-col items-center space-y-2" onClick={() => setActiveSection("merch")}>
          ...
          {activeSection === "schedule" && (<div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Button onClick={() => setNewShowItem({})} className="flex items-center space-x-2" style={{ backgroundColor: colors.primary }}>
              ...
              <div className="grid gap-4">
                {showSchedules?.map((show) => (<Card key={show.id} className={isDarkMode ? "bg-gray-800" : "bg-white"}>
          ...
          {activeSection === "merch" && (<div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Button onClick={() => setNewMerchItem({})} className="flex items-center space-x-2" style={{ backgroundColor: colors.primary }}>
                  <Plus className="w-4 h-4"/>
                  <span>Add Item</span>
                </Button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {mockMerch.map((item) => (<Card key={item.id} className={isDarkMode ? "bg-gray-800" : "bg-white"}>
                    <CardContent className="p-4">
                ...
                <CardContent className="p-6">
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input id="site-title" defaultValue="Spandex Salvation Radio" className="mt-1"/>
                    </div>
                    <div>
                      <Label htmlFor="stream-url">Stream URL</Label>
                      <Input id="stream-url" defaultValue="https://stream.example.com/live" className="mt-1"/>
                    </div>
                    <div>
                      <Label htmlFor="contact-email">Contact Email</Label>
                      <Input id="contact-email" defaultValue="admin@spandex-salvation-radio.com" className="mt-1"/>
                    </div>
