...
export default function SubscribePage() {
    ...
    return (<div className={cn("min-h-screen p-4", theme === "light" ? "bg-white" : "bg-black")}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
          {subscriptionTiers.map((tier) => (<Card key={tier.id} className={cn("relative transition-all duration-300 hover:scale-105", selectedTier === tier.id && "ring-2", tier.popular && "border-2")} style={{
                '--ring-color': selectedTier === tier.id ? colors.primary : undefined,
                borderColor: tier.popular ? colors.primary : undefined,
            }}>
              {tier.popular && (<div className="absolute -top-3 left-1/2 transform -translate-x-1/2 px-4 py-1 rounded-full text-white text-sm font-bold" style={{ backgroundColor: colors.primary }}>
                  MOST POPULAR
                </div>)}
              ...
              <CardContent className="space-y-6">
                ...
                <Button className="w-full" variant={tier.popular ? "default" : "outline"} onClick={() => handleSubscribe(tier.id)} style={tier.popular
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          <Card className="text-center p-6 transition-all duration-300 hover:scale-105 hover:shadow-2xl border-2 cursor-pointer" style={{
