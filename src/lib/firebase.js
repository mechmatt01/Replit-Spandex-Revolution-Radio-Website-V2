...
export async function createUserProfile(firebaseUser, additionalData) {
    try {
...
export async function findUserProfileByFirebaseUID(firebaseUID, email) {
    try {
        // First try to find by Firebase UID
        const q1 = query(collection(db, 'Users'), where('firebaseUID', '==', firebaseUID));
...
export async function getActiveListeners() {
    try {
        const q = query(collection(db, 'Users'), where('isActiveListening', '==', true), where('location', '!=', null));
        const querySnapshot = await getDocs(q);
        const activeListeners = [];
        querySnapshot.forEach((doc) => {
            activeListeners.push(doc.data());
        });
...
export async function getTotalListenersCount() {
    try {
...
export async function updateTotalListenersCount(count) {
    try {
...
export async function getActiveListenersFromFirestore() {
    try {
        const q = query(firestoreCollection(db, "Users"), where("IsActiveListening", "==", true));
        const querySnapshot = await getDocs(q);
        const listeners = [];
        querySnapshot.forEach((doc) => {
            const data = doc.data();
