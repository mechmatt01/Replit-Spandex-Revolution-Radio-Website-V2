...
export function extractCompanyName(metadata: any): string {
  ...
  if (title.length > artist.length && !title.toLowerCase().includes('in a commercial')) {
    return title;
  } else if (artist.length > title.length && !artist.toLowerCase().includes('in a commercial')) {
    return artist;
  }
