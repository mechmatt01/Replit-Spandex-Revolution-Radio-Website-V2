import { detectAdContent } from './adDetection';
...
export class UniversalAdDetector {
    adDetectionCache = new Map();
    cacheTimeout = 30000; // 30 seconds cache
    /**
     * Detect advertisements for any radio station
     */
    async detectAdsForStation(stationId, metadata) {
     ...
     */
    performQuickDetection(metadata, station) {
        if (!metadata) {
     ...
     */
    async performAudioDetection(station, metadata) {
        try {
     ...
     */
    async fetchStationMetadata(station) {
        try {
     ...
     */
    async fetchTritonMetadata(station) {
        if (!station.apiUrl)
            return null;
        try {
     ...
     */
    async fetchStreamTheWorldMetadata(station) {
        if (!station.apiUrl)
            return null;
        try {
            const response = await fetch(station.apiUrl, {
     ...
     */
    async fetchSomaFMMetadata(station) {
        try {
     ...
     */
    async fetchCustomMetadata(station) {
        if (!station.apiUrl)
            return null;
        try {
            const response = await fetch(station.apiUrl, {
     ...
     */
    async autoDetectAndFetchMetadata(station) {
        ...
        for (const apiType of apiTypes) {
            ...
            try {
                ...
                if (metadata) {
                    ...
                    await storage.updateRadioStation(station.id, { apiType: apiType });
                    return metadata;
                }
            }
            catch (error) {
     ...
     */
    createFallbackResult(stationId, isAd) {
        return {
            isAd,
            confidence: 0.5,
            stationId,
