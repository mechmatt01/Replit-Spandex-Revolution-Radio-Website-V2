...
export async function detectAdContent(streamUrl) {
    try {
...
async function captureAudioSample(streamUrl, durationMs) {
...
async function analyzeForAdvertisement(transcription) {
...
export function quickAdDetection(text) {
    if (!text)
        return false;
    const lowerText = text.toLowerCase();
    const adKeywordCount = adKeywords.filter((keyword) => lowerText.includes(keyword)).length;
...
export function analyzeStreamMetadata(metadata) {
    if (!metadata)
        return { isAd: false };
