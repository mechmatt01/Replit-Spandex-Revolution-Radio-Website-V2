...
export async function getCurrentRadioTrack() {
    const now = Date.now();
...
export async function fetchSpotifyArtwork(artist, title) {
...
export async function fetchiTunesArtwork(artist, title) {
    try {
...
export async function getTrackFromLastFM(artist, title) {
    try {
        // Last.fm public API (requires API key for production)
        const response = await fetch(`https://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key=${process.env.LASTFM_API_KEY || 'demo'}&artist=${encodeURIComponent(artist)}&track=${encodeURIComponent(title)}&format=json`);
        if (response.ok) {
            const data = await response.json();
